...
export function lobbyState(code) {
  ...
  const vetoes = db.prepare('SELECT map, banned_by, ord FROM vetoes WHERE code = ? ORDER BY ord').all(code);
  const match = db.prepare('SELECT * FROM matches WHERE code = ? ORDER BY id DESC LIMIT 1').get(code);
...
export async function refreshStats(steamid) {
  try {
    ...
    if (!s) return false;
    db.prepare(`UPDATE players SET premier = ?, leetify_rating = ?, aim = ?, utility = ?,
