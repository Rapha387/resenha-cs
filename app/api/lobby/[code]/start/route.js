...
export async function POST(request, { params }) {
  ...
  const players = db.prepare('SELECT steamid FROM lobby_players WHERE code = ?').all(code);
  if (players.length < 2)
    return NextResponse.json({ erro: 'Precisa de pelo menos 2 jogadores.' }, { status: 400 });

  if (mode === 'auto') {
    const full = players.map(p => db.prepare('SELECT * FROM players WHERE steamid = ?').get(p.steamid));
    full.sort((a, b) => playerScore(b) - playerScore(a));
    let sumA = 0, sumB = 0, countA = 0, countB = 0;
    const half = Math.ceil(full.length / 2);
    const upd = db.prepare('UPDATE lobby_players SET team = ? WHERE code = ? AND steamid = ?');
    for (const p of full) {
      let team;
    ...
    db.prepare('UPDATE lobbies SET status = ?, mode = ?, cap_a = ?, cap_b = ?, turn = ? WHERE code = ?')
  ...
  const restam = db.prepare('SELECT COUNT(*) c FROM lobby_players WHERE code = ? AND team IS NULL').get(code).c;
