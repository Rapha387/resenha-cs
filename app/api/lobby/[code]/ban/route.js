...
export async function POST(request, { params }) {
  ...
  const banned = db.prepare('SELECT map FROM vetoes WHERE code = ?').all(code).map(v => v.map);
  if (banned.includes(map))
    return NextResponse.json({ erro: 'Esse mapa já foi banido.' }, { status: 400 });

  db.prepare('INSERT INTO vetoes (code, map, banned_by, ord) VALUES (?, ?, ?, ?)')
    .run(code, map, user.steamid, banned.length + 1);

  const remaining = MAPS.filter(m => !banned.includes(m.id) && m.id !== map);
  if (remaining.length === 1) {
    db.prepare('UPDATE lobbies SET status = ?, decider_map = ?, turn = NULL WHERE code = ?')
